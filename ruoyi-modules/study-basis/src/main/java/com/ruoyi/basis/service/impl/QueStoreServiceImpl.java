...
import com.ruoyi.basis.mapper.QueStoreMapper;
 ...
 */
@Service
public class QueStoreServiceImpl extends ServiceImpl<QueStoreMapper, QueStore> implements
    IQueStoreService {
  ...
  public List<QueStore> randData(List<String> subjects, Long quantity) {
    ...
    QueryWrapper<QueStore> wrapper = new QueryWrapper<>();
    wrapper.lambda().in(QueStore::getSubject, subjects)
        .last(quantity > 0, "order by rand() limit " + quantity);

    return queStoreMapper.selectList(wrapper);
  }
}
